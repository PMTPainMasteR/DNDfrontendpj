...
--
-- Database: `assignment9_staff_6522781002`
--
CREATE DATABASE IF NOT EXISTS `assignment9_staff_6522781002` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `assignment9_staff_6522781002`;
...
(1, 1, 'Pitchaya', 'Worapaluk', 1, '6522781002@g.siit.tu.ac.th', '6522781002', '6522781002', 4, 0);
...
--
ALTER TABLE `user`
  ADD CONSTRAINT `user_ibfk_1` FOREIGN KEY (`USER_GROUPID`) REFERENCES `usergroup` (`USERGROUP_ID`),
  ADD CONSTRAINT `user_ibfk_2` FOREIGN KEY (`USER_TITLE`) REFERENCES `title` (`TITLE_ID`),
...
(105, 0x41d1b87f9af98c70bd7b8563a8f5542c, 0x5e161e73833e8d634372786319e24da9, 20000, 13000);

--
-- Triggers `account`
--
DELIMITER $$
CREATE TRIGGER `insert_balance` BEFORE UPDATE ON `account` FOR EACH ROW IF NEW.bal < 0 THEN SET NEW.bal = OLD.bal;
SELECT 'Not enough balance' INTO @error;
ELSE
SET @bal = @bal + NEW.bal;
END IF
$$
...
--
-- Stand-in structure for view `account_view_dec`
-- (See below for the actual view)
--
CREATE TABLE `account_view_dec` (
`account_no` varbinary(255)
,`Name` varbinary(255)
,`bal` float
);
...
--
-- Stand-in structure for view `account_view_enc`
-- (See below for the actual view)
--
CREATE TABLE `account_view_enc` (
`account_no` varbinary(255)
,`Name` varbinary(255)
,`balance` float
);
...
CREATE TABLE `transaction` (
  ...
  `accid` varbinary(255) DEFAULT NULL
...
(1, 0x108802d94c983021fdc89dfdba7c7f1e, 14000, '2024-11-11 00:00:00', 0xc24f96adfcc61e35d706a10a417e848c),
(2, 0xb22d8d8b7f159ea5217f5be3cd151592, 13000, '2024-11-11 00:00:00', 0xadc3ddb9b451413d4875f1be180c476d),
(3, 0x97666a6ffa2329a71161c6f61f2b5cb6, 1, '2024-11-11 00:00:00', 0x9a65077c43a87eadac327fae1a8e6f97),
(4, 0xda78768e3808c2452077bee71208c367, 13000, '2024-11-11 00:00:00', 0x38542af7e9fa4e5adc341b089c609284);

--
-- Triggers `transaction`
--
DELIMITER $$
CREATE TRIGGER `insert_sum` BEFORE UPDATE ON `transaction` FOR EACH ROW SET NEW.amount = OLD.amount
$$
DELIMITER ;
...
--
-- Structure for view `account_view_dec`
--
DROP TABLE IF EXISTS `account_view_dec`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `account_view_dec`  AS  select aes_decrypt(`account`.`No`,sha(`account`.`ID`)) AS `account_no`,aes_decrypt(`account`.`Name`,`account`.`ID`) AS `Name`,`account`.`bal` AS `bal` from `account` ;

-- --------------------------------------------------------

--
-- Structure for view `account_view_enc`
--
DROP TABLE IF EXISTS `account_view_enc`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `account_view_enc`  AS  select `account`.`No` AS `account_no`,`account`.`Name` AS `Name`,`account`.`bal` AS `balance` from `account` ;
...
--
-- Database: `bank_lab5_assigment`
--
CREATE DATABASE IF NOT EXISTS `bank_lab5_assigment` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `bank_lab5_assigment`;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `upd_check` BEFORE UPDATE ON `account` FOR EACH ROW BEGIN
...
--
-- Database: `coffee_shop_lab5_assigment`
--
CREATE DATABASE IF NOT EXISTS `coffee_shop_lab5_assigment` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `coffee_shop_lab5_assigment`;
...
`SUM(products.price * order_items.quantity)` decimal(42,2)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `popular_product`  AS  select `products`.`name` AS `name`,`order_items`.`quantity` AS `quantity` from (`order_items` join `products` on((`order_items`.`product_id` = `products`.`product_id`))) order by `order_items`.`quantity` desc,`products`.`name` desc ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `revenue`  AS  select sum((`products`.`price` * `order_items`.`quantity`)) AS `total_revenue` from (`order_items` join `products` on((`order_items`.`product_id` = `products`.`product_id`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `total_revenue`  AS  select sum((`products`.`price` * `order_items`.`quantity`)) AS `SUM(products.price * order_items.quantity)` from (`order_items` join `products` on((`order_items`.`product_id` = `products`.`product_id`))) ;
...
(1, 'John', 'Hagan', 'Senior Programmer', 32, 4, 120000, 25000, 'john_hagan@Hectogen.com'),
(2, 'Ganesh', 'Pillai', 'Senior Programmer', 32, 4, 110000, 20000, 'g_pillai@Hectogen.com'),
(3, 'Anamika', 'Pandit', 'Web\r\nDesigner', 27, 3, 90000, 15000, 'ana@Hectogen.com'),
(4, 'Mary', 'Anchor', 'Web\r\nDesigner', 26, 2, 85000, 15000, 'mary@Hectogen.com'),
(5, 'Fred', 'Kruger', 'Programmer', 31, 3, 75000, 15000, 'fk@Hectogen.com'),
(6, 'John', 'MacFarland', 'Programmer', 34, 4, 80000, 16000, 'john@Hectogen.com'),
(7, 'Edward', 'Sakamuro', 'Programmer', 25, 2, 75000, 14000, 'eddie@Hectogen.com'),
(8, 'Alok', 'Nanda', 'Programmer', 32, 3, 70000, 10000, 'alok@Hectogen.com'),
(9, 'Hassan', 'Rajabi', 'Multimedia Programmer', 33, 3, 90000, 15000, 'hasan@Hectogen.com'),
(10, 'Paul', 'Simon', 'Multimedia\r\nProgrammer', 43, 2, 85000, 12000, 'ps@Hectogen.com'),
(11, 'Arthur', 'Hoopla', 'Multimedia Programmer', 32, 1, 75000, 15000, 'arthur@Hectogen.com'),
(12, 'Kim', 'Hunter', 'Senior Web\r\nDesigner', 32, 2, 110000, 20000, 'kim@Hectogen.com'),
(13, 'Roger', 'Lewis', 'System\r\nAdministrator', 35, 2, 100000, 13000, 'roger@Hectogen.com'),
(14, 'Danny', 'Gibson', 'System\r\nAdministrator', 34, 1, 90000, 12000, 'danny@Hectogen.com'),
(15, 'Mike', 'Harper', 'Senior Marketing\r\nExecutive', 36, 2, 120000, 28000, 'mike@Hectogen.com'),
(16, 'Monica', 'Sehgal', 'Marketing Executive', 30, 3, 90000, 25000, 'monica@Hectogen.com'),
(17, 'Hal', 'Simlai', 'Marketing Executive', 27, 2, 70000, 18000, 'hal@Hectogen.com'),
(18, 'Joseph', 'Irvine', 'Marketing Executive', 27, 2, 72000, 18000, 'joseph@Hectogen.com'),
(19, 'Shahida', 'Ali', 'Customer Service Manager', 32, 3, 70000, 9000, 'shahida@Hectogen.com'),
(20, 'Peter', 'Champion', 'Finance Manager', 36, 4, 120000, 25000, 'peter@Hectogen.com'),
(21, 'Ushik', 'Shrestha', 'Database Instructor', 34, 2, 95000, 17000, 'ushik@Hectogen.com'),
(22, 'David', 'Chan', 'CEO', 28, 4, 200000, 50000, 'david@Hectogen.com');

-- --------------------------------------------------------

--
-- Table structure for table `employee_per`
--
...
--
-- Dumping data for table `employee_per`
--

INSERT INTO `employee_per` (`e_id`, `address`, `phone`, `p_email`, `birth_date`, `sex`, `m_status`, `s_name`, `children`) VALUES
(1, '202, Holder Street', 7176167, 'nettish@hotmail.com', '1972-03-16', 'M', 'Y', 'Anamika Sharma', NULL),
(2, '1232 Marker Hotel Road', 5553312, 'johnny4@hotmail.com', '1968-04-02', 'M', 'Y', 'Jane Donner', 3),
(3, '90 Potter Avenue', 4321211, 'gpillai@youremail.com', '1968-09-22', 'M', 'Y', 'Sandhya Pillai', 2),
(4, '202, Holder Street', 7176167, 'twinkleinmyeyes@hotmail.com', '1972-08-09', 'F', 'Y', 'Manish Sharma', NULL),
(5, 'Apartment #8, Fuhrer Building, Cobb Street', 8973242, 'holychild@heavenlymail.com', '1974-10-13', 'F', 'N', NULL, NULL),
(6, '46 Elm Street', 6666666, 'killeratlarge@elmmail.com', '1969-12-31', 'M', 'N', NULL, NULL),
(7, '432 Mercury Avenue', 7932232, 'macmohan@hotmail.com', '1966-08-20', 'M', 'Y', 'Mary Shelly', 3),
(8, '88 Little Tokyo', 5442994, 'eddies@givememail.com', '1975-01-12', 'M', 'N', NULL, NULL),
(9, '64 Templeton Road', 4327652, 'nandy@physicalemail.com', '1968-05-19', 'M', 'Y', 'Manika Nanda', 1),
(10, '134 Metro House, Handenson Street', 5552376, 'rajabihn@hotmail.com', '1967-07-06', 'M', 'N', NULL, NULL),
(11, '1 Graceland, Aaron Avenue', 5433879, 'soundofsilence@boxer.net', '1957-11-04', 'M', 'Y', 'Muriel Lovelace', 4),
(12, '97 Oakland Road', 5423311, 'kingarthur@roundtable.org', '1968-02-15', 'M', 'Y', 'Rina Brighton', 3),
(13, '543 Applegate Lane', 3434343, 'levy@coolmail.com', '1968-09-03', 'F', 'Y', 'Matt Shikari', 2),
(14, '765 Flasher Street', 7432433, 'tinkertone@email.com', '1965-04-28', 'M', 'N', NULL, NULL),
(15, '98 Gunfoundry', 6500787, 'danny@foolhardy.com', '1966-06-23', 'M', 'Y', 'Betty Cudly', 3),
(16, '#5 Comely Homes', 5432132, 'mikeharper@coldmail.com', '1964-03-06', 'M', 'Y', 'Stella Stevens', 2),
(17, '652 Devon Building, 6th Jake Avenue', 5537885, 'mona@darling.com', '1970-04-18', 'F', 'Y', 'Edgar Alan', 1),
(18, 'Apartment #9, Together Towers', 5476565, 'odessey2000@hotmail.com', '1973-10-09', 'M', 'N', NULL, NULL),
(19, 'Apartment #9, Together Towers', 5476565, 'jirvine@hotteremail', '1973-01-20', 'M', 'N', NULL, NULL),
(20, '90 Comfy Town', 7528326, 'helper@more.org', '1968-01-25', 'F', 'N', NULL, NULL),
(21, '4329 Eucalyptus Avenue', 4254863, 'moneymatters@coldcash.com', '1964-06-13', 'M', 'Y', 'Ruby Richer', 2);
...
--
-- Indexes for table `employee_per`
--
ALTER TABLE `employee_per`
...
--
-- Database: `inventory_managenment_library`
--
CREATE DATABASE IF NOT EXISTS `inventory_managenment_library` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `inventory_managenment_library`;
...
(1, 'Pseudo Harem', 'Yu Saito', 'Comedy, Romantic, School', 0),
(2, 'My One-Hit Kill Sister', 'Konoe/Kenji Taguchi', 'Comedy, Isekai, Adventure', 1),
(3, 'Elusive Samurai', 'Yusei Matsui', 'Action, Drama, Historical', 1);
...
CREATE TABLE `borrowers` (
  ...
  `borrowed_book` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(1, 'Pitchaya Worapaluk', 'Pitchaya.Wor@example.com', 1),
(2, 'Kanphop Puangthong', 'kanphop.pua@example.com', NULL);
...
ALTER TABLE `borrowers`
  ...
  ADD UNIQUE KEY `borrowed_book` (`borrowed_book`);
...
--
ALTER TABLE `borrowers`
  ADD CONSTRAINT `borrowers_ibfk_1` FOREIGN KEY (`borrowed_book`) REFERENCES `book` (`book_id`);
--
-- Database: `lab09`
--
CREATE DATABASE IF NOT EXISTS `lab09` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `lab09`;
...
INSERT INTO `product` (`p_id`, `p_name`, `p_price`, `p_type_id`) VALUES
(1, 'Pencil', 10, 2),
(2, 'Eraser', 5, 2),
(3, 'Mouse', 600, 1),
(4, 'Printer', 4000, 1);
...
INSERT INTO `product_type` (`p_type_id`, `p_type`) VALUES
(1, 'accessories'),
(2, 'stationary');
...
ALTER TABLE `product`
  ...
  ADD KEY `fk_product_type` (`p_type_id`);
...
--
ALTER TABLE `product_type`
  MODIFY `p_type_id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `product`
  ADD CONSTRAINT `fk_product_type` FOREIGN KEY (`p_type_id`) REFERENCES `product_type` (`p_type_id`);
--
-- Database: `lab12`
--
CREATE DATABASE IF NOT EXISTS `lab12` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `lab12`;
--
-- Database: `lib`
--
...
(1, 1, 1, '2023-09-01', '2023-09-27'),
(2, 2, 2, '2023-09-05', '2023-09-20'),
(3, 3, 1, '2023-09-10', '2023-09-01');
...
--
ALTER TABLE `borrower`
  ADD CONSTRAINT `borrower_ibfk_1` FOREIGN KEY (`borrowed_book`) REFERENCES `books` (`book_id`);
--
-- Database: `library_inventory`
--
CREATE DATABASE IF NOT EXISTS `library_inventory` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `library_inventory`;
--
-- Database: `library_lab5_assigment`
--
CREATE DATABASE IF NOT EXISTS `library_lab5_assigment` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `library_lab5_assigment`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `name`  AS  select `authors`.`first_name` AS `first_name`,`authors`.`last_name` AS `last_name` from `authors` group by `authors`.`author_id` order by `authors`.`author_id` ;
...
--
-- Database: `mydb-6522781002`
--
CREATE DATABASE IF NOT EXISTS `mydb-6522781002` DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;
USE `mydb-6522781002`;
...
--
-- Table structure for table `my farm table`
--

CREATE TABLE `my farm table` (
...
--
-- Dumping data for table `my farm table`
--

INSERT INTO `my farm table` (`Animal_ID`, `Animal_Name`, `Animal_Type`, `Animal_DOB`) VALUES
(1, 'Bella', 'Cat', '2020-03-15'),
(2, 'Max', 'Dog', '2018-09-22'),
(3, 'Charlie', 'Rabbit', '2019-05-10'),
(4, 'Luna', 'Cat', '2017-11-03'),
(5, 'Rocky', 'Dog', '2022-11-03'),
(6, 'Daisy', 'Horse', '2015-07-20'),
(7, 'Oliver', 'Cat', '2020-11-28'),
(8, 'Lily', 'Rabit', '2018-04-12'),
(9, 'Lucy', 'Dog', '2016-08-04'),
(10, 'Leo', 'Cat', '2019-02-17');
...
INSERT INTO `my_first_table` (`id`, `firstname`, `lastname`, `age`) VALUES
(1002, 'Pitchaya', 'Worapaluk', 21);
...
INSERT INTO `student` (`Student_ID`, `Student_Name`, `tot-credit`) VALUES
(128, 'Zhang', 23),
(12345, 'Shankar', 23),
(23121, 'Chavez', 24),
(44553, 'Peltier', 26),
...
--
-- Indexes for table `my farm table`
--
ALTER TABLE `my farm table`
...
--
ALTER TABLE `advisor`
  ADD CONSTRAINT `instructor_id_fk` FOREIGN KEY (`instructor_id`) REFERENCES `instructor` (`id`),
  ADD CONSTRAINT `student_id_fk` FOREIGN KEY (`student_id`) REFERENCES `student` (`Student_ID`);
--
-- Database: `mysql_com`
--
CREATE DATABASE IF NOT EXISTS `mysql_com` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `mysql_com`;
...
INSERT INTO `employee` (`id`, `name`, `age`, `dob`, `department`) VALUES
(1, 'Alex', 0, '0000-00-00', 'Software\nArchitect'),
...
INSERT INTO `employee2` (`id`, `firstname`, `lastname`, `city`, `designation`) VALUES
...
(20, 'Peter', 'Champion', 'Finance Manager', 36, 4, 120000, 25000, 'peter@Hectogen.com'),
(21, 'Ushik', 'Shrestha', 'Database Instructor', 34, 2, 95000, 17000, 'ushik@Hectogen.com'),
(22, 'Dave', 'Jones', 'CEO', 28, 4, 240000, 50000, 'david@Hectogen.com');

-- --------------------------------------------------------

--
-- Stand-in structure for view `employee_salary`
-- (See below for the actual view)
--
...
INSERT INTO `product_details` (`product_id`, `product_name`) VALUES
(1, 'Pencils'),
(2, 'PEN'),
(3, 'Cola'),
(4, 'Shirt');
...
INSERT INTO `student` (`id`, `name`, `age`, `dob`, `department`) VALUES
...
INSERT INTO `student2` (`id`, `fname`, `lname`, `age`, `dob`, `department`) VALUES
...
--
-- Stand-in structure for view `total_title`
-- (See below for the actual view)
--
CREATE TABLE `total_title` (
`title` varchar(30)
,`sum(salary)` decimal(32,0)
);

-- --------------------------------------------------------

--
-- Structure for view `employee_salary`
--
DROP TABLE IF EXISTS `employee_salary`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `employee_salary`  AS  select `employee_data`.`f_name` AS `f_name`,`employee_data`.`l_name` AS `l_name`,`employee_data`.`salary` AS `salary` from `employee_data` order by `employee_data`.`salary` desc ;

-- --------------------------------------------------------

--
-- Structure for view `total_title`
--
DROP TABLE IF EXISTS `total_title`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `total_title`  AS  select `employee_data`.`title` AS `title`,sum(`employee_data`.`salary`) AS `sum(salary)` from `employee_data` group by `employee_data`.`title` order by `employee_data`.`title` desc ;
...
--
ALTER TABLE `studentmarks`
  MODIFY `stud_id` smallint(5) NOT NULL AUTO_INCREMENT;
...
-- Database: `mytrpg`
--
CREATE DATABASE IF NOT EXISTS `mytrpg` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `mytrpg`;
...
(14, 'หมู่บ้านเฮี้ยนตะเคียนหลอน', 'ระทึกขวัญ, สยองขวัญ', 'Ongoing', 5, 'เรื่องราวมันเริ่มต้นขึ้นจากการที่ผู้ใหญ่บ้านต้องการ จะตัดต้นตะเคียนแต่ก็ไม่สามารถตัดได้');
...
--
ALTER TABLE `cattack`
  MODIFY `CharacterAttackID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;

--
-- AUTO_INCREMENT for table `cgear`
--
ALTER TABLE `cgear`
  MODIFY `CharacterGearID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;

--
-- AUTO_INCREMENT for table `characters`
--
ALTER TABLE `characters`
  MODIFY `CharacterID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

--
-- AUTO_INCREMENT for table `cinventory`
--
ALTER TABLE `cinventory`
  MODIFY `CharacterInventoryID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;

--
-- AUTO_INCREMENT for table `cspelltalent`
--
ALTER TABLE `cspelltalent`
  MODIFY `CharacterSpellTalentID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
-- Stand-in structure for view `customer_salary_view`
-- (See below for the actual view)
--
CREATE TABLE `customer_salary_view` (
`NAME` varchar(255)
,`SALARY` varbinary(16)
);
...
--
-- Stand-in structure for view `customer_salary_view_de`
-- (See below for the actual view)
--
CREATE TABLE `customer_salary_view_de` (
`NAME` varchar(255)
,`AES_DECRYPT(salary,SHA1('salary'))` varbinary(16)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `customer_salary_view_dec`
-- (See below for the actual view)
--
CREATE TABLE `customer_salary_view_dec` (
`NAME` varchar(255)
,`SALARY` varbinary(16)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customers_view`  AS  select `customers`.`ID` AS `ID`,`customers`.`NAME` AS `NAME`,`customers`.`AGE` AS `AGE`,`customers`.`ADDRESS` AS `ADDRESS`,`customers`.`SALARY` AS `SALARY` from `customers` ;

-- --------------------------------------------------------

--
-- Structure for view `customer_salary_view`
--
DROP TABLE IF EXISTS `customer_salary_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_salary_view`  AS  select `customers`.`NAME` AS `NAME`,aes_encrypt(`customers`.`SALARY`,sha('salary')) AS `SALARY` from `customers` ;

-- --------------------------------------------------------

--
-- Structure for view `customer_salary_view_de`
--
DROP TABLE IF EXISTS `customer_salary_view_de`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_salary_view_de`  AS  select `customer_salary_view`.`NAME` AS `NAME`,aes_decrypt(`customer_salary_view`.`SALARY`,sha('salary')) AS `AES_DECRYPT(salary,SHA1('salary'))` from `customer_salary_view` ;

-- --------------------------------------------------------

--
-- Structure for view `customer_salary_view_dec`
--
DROP TABLE IF EXISTS `customer_salary_view_dec`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `customer_salary_view_dec`  AS  select `customer_salary_view`.`NAME` AS `NAME`,aes_decrypt(`customer_salary_view`.`SALARY`,sha('salary')) AS `SALARY` from `customer_salary_view` ;
...
INSERT INTO `register` (`Reg_ID`, `Student_ID`, `Section_ID`, `Registeration_date`, `Grade`) VALUES
(1, 1, 1, '2023-09-01', 'B+'),
(2, 2, 2, '2023-09-02', 'B+'),
(3, 3, 3, '2023-09-03', 'A-'),
(4, 4, 4, '2023-09-04', 'B+'),
...
INSERT INTO `student` (`Student_ID`, `f_name`, `l_name`, `email`, `DOB`, `Gender`) VALUES
...
--
ALTER TABLE `register`
  MODIFY `Reg_ID` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
...
ALTER TABLE `register`
  ...
  ADD CONSTRAINT `register_ibfk_1` FOREIGN KEY (`Section_ID`) REFERENCES `section` (`Section_ID`);
...
--
-- Database: `school_6522781002`
--
CREATE DATABASE IF NOT EXISTS `school_6522781002` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `school_6522781002`;
...
(105, 'English Literature', 'English'),
(106, 'Data Sturctures', 'Computer Science'),
(107, 'Linear Algebra', 'Mathematics'),
(108, 'Organic Chemistry', 'Chemistry'),
(109, 'Macroesconomics', 'Economics'),
(110, 'World History', 'History');
...
INSERT INTO `register` (`reg_id`, `std_id`, `sec_id`, `reg_date`, `gpa`) VALUES
(1, 1, 1, '2023-09-01', 'A'),
(2, 2, 2, '2023-09-02', 'B+'),
(3, 3, 3, '2023-09-03', 'A-'),
(4, 4, 4, '2023-09-04', 'B'),
...
INSERT INTO `section` (`sec_id`, `course_id`, `sem`, `year`, `instructor`) VALUES
(1, 101, 'Fall', 2023, 'Dr. Smith'),
(2, 102, 'Spring', 2024, 'Prof. Johnson'),
(3, 103, 'Fall', 2023, 'Dr. Lee'),
...
INSERT INTO `student` (`std_id`, `fname`, `lname`, `email`, `DOB`, `Gender`) VALUES
(1, 'John', 'Doe', 'john.doe@example.com', '2000-01-15', 'M'),
(2, 'Jane', 'Smith', 'jane.smith@example.com', '2001-02-20', 'F'),
(3, 'Alice', 'Johnson', 'alice.johnson@example.com', '1999-03-25', 'F'),
(4, 'Bob', 'Williams', 'bob.williams@example.com', '2002-04-30', 'M'),
(5, 'Charlie', 'Brown', 'charlie.brown@example.com', '2000-05-05', 'M'),
(6, 'Emily', 'Davis', 'emily.davis@example.com', '2001-06-10', 'F'),
(7, 'Frank', 'Miler', 'frank.miler@example.com', '1998-07-05', 'M'),
(8, 'Grace', 'Wilson', 'grace.wilson@example.com', '1999-08-20', 'F'),
(9, 'Henry', 'Moore', 'henry.moore@example.com', '2000-09-25', 'M'),
(10, 'Ivy', 'Taylor', 'ivy.taylor@example.com', '2001-10-30', 'F');
...
ALTER TABLE `register`
  ADD PRIMARY KEY (`reg_id`),
  ADD KEY `Reg_std_id_fk` (`std_id`),
  ADD KEY `Reg_sec_id_fk` (`sec_id`);
...
ALTER TABLE `section`
  ...
  ADD KEY `sec_course_id_fk` (`course_id`);
...
--
ALTER TABLE `register`
  ADD CONSTRAINT `Reg_sec_id_fk` FOREIGN KEY (`sec_id`) REFERENCES `section` (`sec_id`),
  ADD CONSTRAINT `Reg_std_id_fk` FOREIGN KEY (`std_id`) REFERENCES `student` (`std_id`);

--
-- Constraints for table `section`
--
ALTER TABLE `section`
  ADD CONSTRAINT `sec_course_id_fk` FOREIGN KEY (`course_id`) REFERENCES `course` (`course_id`);
--
-- Database: `school_assignment5`
--
CREATE DATABASE IF NOT EXISTS `school_assignment5` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `school_assignment5`;
...
--
ALTER TABLE `salary`
  ADD CONSTRAINT `salary_ibfk_1` FOREIGN KEY (`instructor_id`) REFERENCES `instructor` (`Instructor_ID`),
  ADD CONSTRAINT `salary_ibfk_2` FOREIGN KEY (`dept_code`) REFERENCES `department` (`dept_code`);
...
--
-- Database: `school_lab_assignment5`
--
CREATE DATABASE IF NOT EXISTS `school_lab_assignment5` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `school_lab_assignment5`;
...
--
-- Database: `search_engine_lab5_assigment`
--
CREATE DATABASE IF NOT EXISTS `search_engine_lab5_assigment` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `search_engine_lab5_assigment`;
...
--
-- Database: `staff_6522781002`
--
CREATE DATABASE IF NOT EXISTS `staff_6522781002` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `staff_6522781002`;
...
INSERT INTO `usergroup` (`usergroup_id`, `usergroup_code`, `usergroup_remark`, `usergroup_url`) VALUES
(1, 'Admin', 'Administrator', 'admin_view.php'),
(2, 'Staff', 'Staff', 'staff_view.php'),
(3, 'Member', 'Member', 'member_view.php'),
(4, 'Student', 'Student Member', '');
...
ALTER TABLE `user`
  ...
  ADD CONSTRAINT `user_ibfk_2` FOREIGN KEY (`user_groupid`) REFERENCES `usergroup` (`usergroup_id`),
  ADD CONSTRAINT `user_ibfk_3` FOREIGN KEY (`user_title`) REFERENCES `title` (`title_id`);
--
-- Database: `test`
--
...
CREATE TABLE `student` (
  `Student_ID` int(11) NOT NULL,
  `Student_Name` varchar(25) NOT NULL DEFAULT 'Bipul Neupane'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
--
-- Database: `test_lab_06`
--
CREATE DATABASE IF NOT EXISTS `test_lab_06` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `test_lab_06`;
...
INSERT INTO `login` (`User_ID`, `Singup_ID`, `Username`, `Password`) VALUES
(14, 56, 'bhoon', '6522781002');
...
--
ALTER TABLE `login`
  ADD CONSTRAINT `login_ibfk_1` FOREIGN KEY (`Singup_ID`) REFERENCES `signup` (`ID`) ON DELETE CASCADE;
--
-- Database: `test_school`
--
CREATE DATABASE IF NOT EXISTS `test_school` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `test_school`;
